...
export function nextPointer(current) {
    const idx = POINTERS.indexOf(current);
    if (idx < 0 || idx >= POINTERS.length - 1)
        return null;
...
export function checkLedgerCaughtUp(dir, state) {
    const errors = [];
    if (state.history_stack.length <= 1)
        return { ok: true, errors };
    const last = lastLedgerTransition(dir);
    if (!last || last.to !== state.pointer) {
        errors.push(`ledger.md is not caught up: expected the last transition entry to end at '${state.pointer}', ` +
...
export function checkGateBuild(scopeArg, specRefs) {
    const errors = [];
...
export function checkGateDesignReconfirm(dir, tier) {
    const errors = [];
    if (tier !== 'T3')
        return { ok: true, errors };
    ...
    if (!last || last.from !== 'decide' || last.to !== 'decide' || !last.satisfied.includes('confirmed_by_user:true')) {
        errors.push('T3 requires an explicit design reconfirmation before entering build: after getting the user\'s ' +
...
export function checkArchiveReady(proposal, state) {
    const errors = [];
...
export const VALID_ITEM_TRANSITIONS = {
...
export function canTransitionItem(state, itemId, to) {
