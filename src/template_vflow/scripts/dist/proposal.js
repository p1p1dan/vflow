...
import { nextProposalId, proposalDir, findProposalDir, readProposal, writeProposal, readRepoIndex, upsertRepoIndex, readState, writeState, initLedger, findItem, setItemStatus, } from './lib/store.js';
import { nextPointer, checkLedgerCaughtUp, checkGateBuild, checkGateDesignReconfirm, checkArchiveReady, canTransitionItem, } from './lib/checks.js';
...
function repoIndexEntry(dir, proposal) {
    return {
...
function cmdMove(flags) {
    ...
    const state = readState(dir);
    if (!state) {
        console.log(`state.json not found in '${dir}'.`);
        return;
    }
    const to = flags['to'];
    if (!to || !POINTERS.includes(to)) {
    ...
    if (!ledgerCheck.ok) {
        ...
        for (const e of ledgerCheck.errors)
            console.log(`  - ${e}`);
        return;
    }
    if (to === 'build') {
...
function cmdItem(sub, positional, flags) {
    const resolved = resolveProposal(flags);
    if (!resolved)
        return;
    const { dir, proposal } = resolved;
    if (proposal.tier === 'T1') {
        console.log('T1 proposals have no state.json/items.');
        return;
    }
    const state = readState(dir);
    if (!state) {
    ...
    switch (sub) {
        case 'add': {
            ...
            if (!title) {
                console.log('Usage: item add --title "..."');
                return;
            }
        ...
        case 'start': {
            ...
            if (!itemId) {
                console.log('Usage: item start --item E-001');
                return;
            }
        ...
        case 'complete': {
            const itemId = flags['item'] ?? positional[0];
            if (!itemId) {
                console.log('Usage: item complete --item E-001 [--note "..."]');
                return;
            }
        ...
        case 'block': {
            const itemId = flags['item'] ?? positional[0];
            if (!itemId) {
                console.log('Usage: item block --item E-001 [--note "..."]');
                return;
            }
        ...
        case 'cancel': {
            const itemId = flags['item'] ?? positional[0];
            if (!itemId) {
                console.log('Usage: item cancel --item E-001');
                return;
            }
...
function cmdSpecRef(sub, flags) {
    const resolved = resolveProposal(flags);
    if (!resolved)
        return;
    const { dir, proposal } = resolved;
    if (proposal.tier === 'T1') {
        console.log('T1 proposals have no state.json/spec_refs.');
        return;
    }
    const state = readState(dir);
    if (!state) {
    ...
    if (sub === 'add') {
        ...
        if (!file || !reason) {
            console.log('Usage: spec-ref add --file <path> --reason "..."');
            return;
        }
        ...
        const reason = flags['reason'];
        if (!reason) {
            console.log('Usage: spec-ref none --reason "..."');
            return;
        }
...
async function cmdAccept(flags) {
    ...
    // Write audit trail
    const acceptedSource = userApproved ? 'ai_relay' : 'user_terminal';
...
function cmdKnowledge(sub, flags) {
    if (sub === 'suggest') {
        ...
        import('./lib/derive.js').then(({ knowledgeSuggestGuidance }) => {
            for (const line of knowledgeSuggestGuidance())
                console.log(line);
        });
    }
    else if (sub === 'save') {
...
switch (cmd) {
    ...
    case 'move':
        ...
        break;
    case 'item':
        cmdItem(sub, positional, flags);
        break;
    case 'spec-ref':
        cmdSpecRef(sub, flags);
        break;
    case 'set':
        cmdSet(sub, flags);
        break;
