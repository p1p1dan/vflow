...
在处理用户第一条消息时，必须先询问是否启用（仅问一次）：
- 用户第一条消息是开发/修改类任务 → 先用 AskUserQuestion 工具询问"是否为本项目启用 vflow 工作流？"
  （选项：启用（推荐）/ 本项目不启用），得到答案后再开始处理任务
- 用户第一条消息是纯问答/闲聊 → 在回复的第一句先问"本项目还未启用 vflow 工作流，需要启用吗？"，
  然后正常回答其问题
处理用户的选择：
- 同意 → 执行 `vflow init . --yes`（静默默认配置），完成后建议运行 /vflow:init 让 AI 探测项目配置
- 拒绝 → 执行 `vflow decline .`（记录后不再询问；将来想启用说一声"启用 vflow"即可）
- 用户跳过/未回应 → 不追问，本会话不再提及
</vflow-suggest>"""
