...
import { writeFileSync, existsSync, mkdirSync, readdirSync, appendFileSync, unlinkSync, statSync, } from 'node:fs';
import { join, dirname, basename } from 'node:path';
import { spawnSync } from 'node:child_process';
import { createHash } from 'node:crypto';
import { hostname } from 'node:os';
import { readJson, readText, isoNow, isoToday, ROOT, TASKS, RUNTIME, } from './lib/config.js';
// ---------------------------------------------------------------------------
// Path constants
// ---------------------------------------------------------------------------
...
function cmdJoin(args) {
    ...
    const role = args.role || (isFirst ? 'admin' : 'member');
    const member = {
        uid,
        name: name || uid,
        email,
...
function cmdWhoami() {
    ...
    const member = existsSync(memberFile) ? readJson(memberFile) ?? self : self;
...
function cmdHeartbeat() {
    ...
    if (existsSync(dedupFile)) {
        try {
            ...
            if (Date.now() - lastTs < DEDUP_WINDOW_MS)
                return 0;
        }
        catch { /* proceed */ }
    }
...
function cmdClaim(args) {
    ...
    if (!slug) {
        console.log('Usage: collab.js claim <slug>');
        return 1;
    }
    ...
    const self = resolveSelf();
    if (!self) {
        console.log('[collab] Cannot determine identity');
        return 1;
    }
...
function cmdRelease(args) {
    ...
    const slug = args.slug;
    if (!slug) {
        console.log('Usage: collab.js release <slug>');
        return 1;
    }
...
function cmdStaging(args) {
    ...
    const cfg = readJson(CONFIG, {});
    if (cfg?.team && cfg.team.spec_review === false) {
...
function cmdReview(args) {
    ...
    if (decision === 'approve') {
        ...
        if (!targetPath.startsWith('/') && !targetPath.includes(':')) {
            if (targetPath.startsWith('spec/'))
                targetPath = join(ROOT, targetPath);
            else
                targetPath = join(ROOT, 'spec', targetPath);
        }
...
function cmdSearch(args) {
    ...
    if (!query) {
        console.log('Usage: collab.js search <query>');
        return 1;
    }
    const archiveDir = join(TASKS, 'archive');
    if (!existsSync(archiveDir)) {
        console.log('[collab] No archive directory');
        return 0;
    }
    ...
    let monthDirs;
    try {
        monthDirs = readdirSync(archiveDir).sort().reverse();
    }
    catch {
        return 0;
    }
...
function parseArgs() {
    ...
    function getOption(names, defaultVal = '') {
        for (let i = 0; i < rest.length; i++) {
            if (names.includes(rest[i]) && i + 1 < rest.length)
                return rest[i + 1];
        }
        return defaultVal;
    }
    function positional(index) {
        ...
        for (let i = 0; i < rest.length; i++) {
            if (rest[i].startsWith('-')) {
                if (['--role', '--window', '--task', '--target', '--action', '--summary', '--impl'].includes(rest[i]))
                    i++;
                continue;
            }
            if (pos === index)
