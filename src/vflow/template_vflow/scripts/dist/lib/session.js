...
export function resolveCliSessionKey() {
...
export function readSessionBinding(sessionKey) {
    const data = readSessionFile(sessionFilePath(sessionKey));
    const t = data?.current_task;
    return typeof t === 'string' && t.trim() ? t.trim() : null;
}
export function writeSessionBinding(sessionKey, taskSlug) {
...
export function clearTaskFromSessions(taskSlug) {
    let cleared = 0;
...
export function resolveCurrentTaskSlug(payload, pointerFallback) {
    const sessionKey = resolveSessionKey(payload);
    if (sessionKey) {
        const bound = readSessionBinding(sessionKey);
        if (bound)
            return { slug: bound, source: 'session' };
    }
    ...
    if (ptr)
        return { slug: ptr, source: 'pointer' };
...
export function resolveCliTaskSlug(pointerFallback) {
