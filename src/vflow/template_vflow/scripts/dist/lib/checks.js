...
import { isFilled, uncheckedItems, designPath, requirementPath, verifyPath, latestWorklogMtime, isV2Docs, parseSection, } from './docs.js';
import { runVerification } from './verify.js';
// ---------------------------------------------------------------------------
// R-ID source resolution
// ---------------------------------------------------------------------------
/** Read R-ID definitions from the appropriate document. */
function ridSource(taskDir) {
    if (isV2Docs(taskDir)) {
...
// Transition checks
// ---------------------------------------------------------------------------
export function checkAnalyzed(taskDir, _cfg, _task) {
    const errors = [];
...
export function checkDesigned(taskDir, _cfg, _task) {
...
export function checkImplementing(_taskDir, _cfg, _task) {
    return [];
}
export function checkVerified(taskDir, cfg, task) {
    const errors = [];
...
export function checkArchived(taskDir, cfg, task) {
    const errors = [];
    ...
    if (verifiedAt) {
        ...
        if (latest > vts) {
            ...
            errors.push(`source files changed after verification (${latestIso}). Run 'task.ts back' then 'task.ts advance' to re-verify.`);
        }
    }
    else {
        const testRequired = cfg.test_required !== undefined ? cfg.test_required : true;
        if (testRequired) {
            errors.push("no machine verification record (verified_at missing); run 'task.ts back' then 'task.ts advance' to verify with tests");
        }
    }
    return errors;
}
