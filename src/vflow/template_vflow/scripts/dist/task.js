...
import { readJson, readText, isoNow, monthDay, ROOT, TASKS, RUNTIME, POINTER, CONFIG_PATH, STATES, } from './lib/config.js';
...
function resolveTaskDir(explicitSlug) {
    if (explicitSlug) {
...
function cmdAdvance(args) {
    ...
    if (!d) {
        console.log('[vflow] No active task');
        return 1;
    }
    ...
    if (state === 'archived') {
        console.log('[vflow] Task already archived');
        return 1;
    }
    if (state === 'verified') {
        console.log('[vflow] Next step is archival: run task.js done --summary "..."');
        return 1;
    }
...
function cmdStatus(args) {
    const d = resolveTaskDir(args.task);
    if (!d) {
        console.log('[vflow] No active task (no_task)');
        return 0;
    }
...
function parseArgs() {
        ...
        for (let i = 0; i < rest.length; i++) {
            if (rest[i].startsWith('-')) {
                if (['--title', '--tier', '--summary', '--impl', '--files', '--task',
                    '--status', '--evidence', '--concerns', '--reason'].includes(rest[i]))
                    i++;
                continue;
            }
            if (pos === index)
    ...
    switch (cmd) {
        ...
        case 'set': {
            ...
            if (!key || !value) {
                console.log('Usage: task.js set <key> <value>');
                process.exit(1);
            }
        ...
        case 'quick-log': {
            ...
            if (!title) {
                console.log('Usage: task.js quick-log --title "..." [--files a.ts,b.ts]');
                process.exit(1);
            }
...
function main() {
    ...
    const handlers = {
        ...
        advance: (a) => cmdAdvance(a),
        back: (a) => cmdBack(a),
        set: (a) => cmdSet(a),
        start: (a) => cmdStart(a),
        done: (a) => cmdDone(a),
        status: (a) => cmdStatus(a),
        followup: (a) => cmdFollowup(a),
        'quick-log': (a) => cmdQuickLog(a),
        trace: (a) => cmdTrace(a),
        next: (a) => cmdNext(a.task || undefined),
