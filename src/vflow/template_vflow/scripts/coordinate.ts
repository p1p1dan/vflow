...
import type { SpawnFn } from './lib/cli-executor.js';
...
import { ROOT, GRAPHS_DIR } from './lib/config.js';
...
async function createWalker(workflowRoot: string) {
  ...
  const walker = new GraphWalker(
    ...
    null, // no step analyzer
    parser, evaluator,
    undefined, // no event emitter
    sessionDir,
    undefined, // no parallel executor (for now)
    null, // no LLM decider
...
function parseCliArgs(): { subcommand: string; args: string[]; opts: Record<string, string | boolean> } {
...
async function main(): Promise<void> {
  ...
  const workflowRoot = resolve(opts['workflow-root'] as string ?? process.cwd());
  ...
  switch (subcommand) {
    ...
    case 'start': {
      ...
      const graphId = opts['graph'] as string;
      if (!graphId) {
    ...
    case 'run': {
      ...
      if (opts['continue']) {
        ...
        if (opts['dry-run']) console.error('[coordinate] Dry-run mode');
    ...
    case 'report': {
      ...
      const node = opts['node'] as string;
      const status = (opts['status'] as string)?.toUpperCase();
      ...
      const payload = {
        status,
        phase: (opts['phase'] as string) ?? null,
        verification_status: (opts['verification'] as string) ?? null,
        review_verdict: (opts['review'] as string) ?? null,
        uat_status: (opts['uat'] as string) ?? null,
        artifacts: (opts['artifact'] as string)?.split(',') ?? [],
