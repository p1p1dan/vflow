...
import type { TaskJson, VflowConfig } from './lib/config.js';
import {
  readJson, readText, ROOT, TASKS, RUNTIME,
  STATES,
} from './lib/config.js';
...
function readConfigFlags(cfg: VflowConfig): ConfigFlags {
  const enabled = cfg.enabled;
  return {
    enabled: enabled !== false,
    initialized: (cfg as unknown as Record<string, unknown>).initialized === true,
  };
}
...
interface FollowupEntry {
  source: string;
  title: string;
  pending: Array<{ priority: string; id: string; title: string; done: boolean }>;
}

function scanPendingFollowups(): FollowupEntry[] {
  ...
  for (const month of monthDirs) {
    ...
    for (const tname of tasks) {
      const t = readJson<TaskJson>(join(mdir, tname, 'task.json'));
...
function formatFollowupBlock(followups: FollowupEntry[]): string {
...
function currentState(): [string, TaskJson | null, string | null] {
  ...
  try {
    ...
    const t = JSON.parse(raw) as TaskJson;
...
function specManifest(taskDir: string | null): Array<[string, string]> {
  if (!taskDir) return [];
...
function loadSpecContents(entries: Array<[string, string]>): Array<[string, string, string]> {
...
function pipelineLine(state: string): string {
...
function scanTeamActivity(windowMin = 30): Array<Record<string, unknown>> {
...
function formatTeamBlock(cfg: VflowConfig): string {
  ...
  if (Object.keys(byUser).length === 0) {
    ...
    for (const [user, events] of Object.entries(byUser)) {
      ...
      const ago = Math.round((Date.now() - new Date(latest.ts as string).getTime()) / 60000);
