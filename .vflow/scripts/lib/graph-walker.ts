...
import type { WorkflowHookRegistry } from './hooks.js';
...
export class GraphWalker {
  ...
  // `vflow coordinate report`, the structured result is already on disk
  ...
  private loadState(sessionId?: string): WalkerState {
    ...
    const dirs = entries
      .filter((e: { isDirectory(): boolean; name: string }) => e.isDirectory() && e.name.startsWith('coord-'))
