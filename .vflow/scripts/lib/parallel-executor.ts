...
// ---------------------------------------------------------------------------
// ParallelRunner — generic runner interface (injected dependency)
// ---------------------------------------------------------------------------

export interface ParallelRunner {
  runAll(
    tasks: Array<{ id: string; prompt: string; tool: string; workDir: string; mode: string }>,
    opts: { joinStrategy: string; signal?: AbortSignal },
  ): Promise<{ results: Array<{ id: string; success: boolean; output: string; durationMs: number }> }>;
}
...
export class DefaultParallelExecutor implements ParallelCommandExecutor {
  constructor(private readonly runner: ParallelRunner) {}
